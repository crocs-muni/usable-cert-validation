...
def set_signature_algorithm_to_md5(tbs):
    tbs['signature'] = x509.algorithm_identifier('md5WithRSAEncryption')
...
def main():
    ...
    set_signature_algorithm_to_md5(end_tbs)
    end_tbs_der = asn.encode('TBSCertificate', end_tbs)

    # Sign the end entity tbsCertificate
    end_sig = crypto.rsa_md5_sign(sub_privkey, end_tbs_der)
