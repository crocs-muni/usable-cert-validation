...
def ip_addr_blocks_extension(asn):
    permitted_prefix = '126.0'
    return x509.ip_addr_blocks([permitted_prefix], asn)


def ip_addr_blocks_no_subset(asn):
    permitted_prefix = '127.0.0.1'
    return x509.ip_addr_blocks([permitted_prefix], asn)
...
def main():
    ...
    # Create ip-addr-blocks extenstion for intermediate CA
    ip_ext = ip_addr_blocks_extension(asn)
    ...
    # Create SAN extenstion for intermediate CA
    ip_ext_end = ip_addr_blocks_no_subset(asn)
    ...
    end_tbs = x509.default_tbs(issuer_public_key=sub_pubkey,
                               ...
                               is_ca=False,
                               additional_extensions=[ip_ext_end],
                               asn=asn)
