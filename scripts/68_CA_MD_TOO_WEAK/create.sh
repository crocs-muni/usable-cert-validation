...
certtool --generate-certificate --load-request files/user.csr --outfile files/user.crt --load-ca-certificate files/ca.crt --load-ca-privkey files/ca.key --template user.cfg --hash SHA1
