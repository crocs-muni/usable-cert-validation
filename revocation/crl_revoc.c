...
static bool add_trusted_CAs_into_trusted_list(gnutls_x509_trust_list_t trusted_list)
{
    int ret_err_val;
    /* Another options: 'gnutls_x509_trust_list_add_cas', 'gnutls_x509_trust_list_add_trust_file'. */
    if ((ret_err_val = gnutls_x509_trust_list_add_system_trust(trusted_list, 0, 0)) <= 0)
    {
        fprintf(stderr, "Function 'gnutls_x509_trust_list_add_system_trust' has failed: %s\n", gnutls_strerror(ret_err_val));
        exit(EXIT_FAILURE);
    }
...
static bool add_trusted_CRLs_into_trusted_list(gnutls_x509_trust_list_t trusted_list, gnutls_x509_crt_t certificate, gnutls_x509_crt_t issuer_certificate) {
    int ret_error_val;
    CURL *handle = NULL;
    ...
    /* Prepare the native gnutls_x509_crl_t structure where the downloaded CRL will be placed */
    if ((ret_error_val = gnutls_x509_crl_init(&downloaded_crl)) != GNUTLS_E_SUCCESS) {
    ...
    while (1) {
        ...
        ret_error_val = gnutls_x509_crt_get_crl_dist_points(certificate, dist_points_index, buffer_crl_dist_point, &buffer_crl_dist_point_size, &revocation_reasons, NULL);
        ...
        if ((ret_error_val = gnutls_x509_crl_import(downloaded_crl, &downloaded_crl_DER, GNUTLS_X509_FMT_DER)) != 0)
        {
        ...
        /* After downloading, check the signature of the downloaded CRL (if CRL was signed by the CA which signed the certificate) */
        ...
        {
            fprintf(stderr, "Function 'gnutls_x509_trust_list_add_trust_mem' added nothing to the trusted list!\n");
            gnutls_free(downloaded_crl_DER.data);
            goto cleanup;
        }
...
static bool verify_server_chain_against_trusted_list(gnutls_x509_trust_list_t trusted_list, gnutls_x509_crt_t *server_chain_crt, size_t server_chain_size)
{
    int ret_error_val;

    /* Verify the server's chain against filled trusted list */
    /* Possible to modify behaviour with 4th argument which is enum of gnutls_certicate_cerify_flags (GNUTLS_VERIFY_*). */
    /* ORed sequence of gnutls_certificate_status_t enum */
    unsigned int verify_output;
    if ((ret_error_val = gnutls_x509_trust_list_verify_crt(trusted_list, server_chain_crt, server_chain_size, 0, &verify_output, NULL)) != 0)
    {
        fprintf(stderr, "Function 'gnutls_x509_trust_list_verify_crt' has failed: %s\n", gnutls_strerror(ret_error_val));
        exit(EXIT_FAILURE);
    }
...
bool crl_revoc_check(gnutls_session_t session)
{
    ...
    gnutls_x509_crt_t *server_chain_crt = retrieve_server_certificate_chain(session, &server_chain_size);
    if (server_chain_crt == NULL) {
        exit(EXIT_FAILURE);
    }

    /* Prepare the trust list structure */
    /* This structure is gonna be filled with system's default trusted CAs and trusted verified CRLs which we gonna download in next steps */
    ...
    /* b) Download all CRLs from CRL Distribution Point extension, verify them and then add to the trusted list */
    for (int index = 0; index < server_chain_size - 1; index++) {
        printf("Downloading CRLs of certificate at index %d\n", index);
        if (!add_trusted_CRLs_into_trusted_list(trusted_list, server_chain_crt[index], server_chain_crt[index + 1])) {
            goto cleanup;
        }
        printf("\n");
    }
    ...
    if (!verify_server_chain_against_trusted_list(trusted_list, server_chain_crt, server_chain_size)) {
        goto cleanup;
    }
