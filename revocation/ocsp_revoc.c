 ...
 * gnutls_ocsp_resp_status_t  -- GNUTLS_OCSP_RESP_*  -- from gnutls_ocsp_resp_get_status
 * gnutls_ocsp_cert_status_t -- GNUTLS_OCSP_CERT_*
...
static char *extract_ocsp_responder_uri(gnutls_x509_crt_t certificate)
{
    ...
    while (1)
    {
        /* Parse the URL adress of the certificate from its extension called authority info access */
        ret_val = gnutls_x509_crt_get_authority_info_access(certificate, act_index, GNUTLS_IA_OCSP_URI, &ocsp_responder_uri_datum, NULL);
...
static gnutls_ocsp_req_t generate_ocsp_request(gnutls_x509_crt_t certificate, gnutls_x509_crt_t issuer_certificate, gnutls_datum_t *_nonce)
{
    int ret_error_val;
    ...
    /* Add the serial number of the certificate we want to check, its issuer name and key */
    ...
    if ((ret_error_val = gnutls_ocsp_req_add_cert(ocsp_req, GNUTLS_DIG_SHA1, issuer_certificate, certificate)) != GNUTLS_E_SUCCESS)
    {
    ...
    if ((ret_error_val = gnutls_ocsp_req_randomize_nonce(ocsp_req)) != GNUTLS_E_SUCCESS)
    {
        fprintf(stderr, "Function 'gnutls_ocsp_req_randomize_nonce' has failed: %s\n", gnutls_strerror(ret_error_val));
        gnutls_ocsp_req_deinit(ocsp_req);
        exit(EXIT_FAILURE);
    }
    ...
    if ((ret_error_val = gnutls_ocsp_req_get_nonce(ocsp_req, NULL, &nonce_req)) != GNUTLS_E_SUCCESS)
    {
...
static gnutls_ocsp_resp_t send_generated_ocsp_request(gnutls_ocsp_req_t ocsp_req, char *ocsp_responder_uri)
{
    int ret_val;
    CURL *handle = NULL;
    ...
    if ((ret_val = gnutls_ocsp_resp_import(ocsp_response, &ocsp_response_datum)) != GNUTLS_E_SUCCESS)
    {
...
bool verify_ocsp_response_signature(gnutls_ocsp_resp_t ocsp_response, gnutls_x509_crt_t certificate, gnutls_x509_crt_t issuer_certificate, gnutls_datum_t nonce_req)
{
    int ret_value;
    ...
    if (gnutls_ocsp_resp_check_crt(ocsp_response, 0, certificate) != GNUTLS_E_SUCCESS)
    {
        fprintf(stderr, "- ocsp response does not contain correct certificate's serial number");
        return false;
    }
    ...
    if (gnutls_ocsp_resp_get_nonce(ocsp_response, NULL, &nonce_resp) == GNUTLS_E_SUCCESS)
    {
        /* Check that the nonces from the OCSP Request and OCSP Response are the same */
        if (nonce_req.size != nonce_resp.size || memcmp(nonce_req.data, nonce_resp.data, nonce_resp.size) != 0)
        {
            fprintf(stderr, "- nonce OCSP Req and nonce OCSP Resp are not the same\n");
        }
    ...
    if ((ret_value = gnutls_ocsp_resp_verify_direct(ocsp_response, issuer_certificate, &verify_result, 0)) != GNUTLS_E_SUCCESS)
    {
        fprintf(stderr, "Function 'gnutls_ocsp_resp_verify_direct' has failed: %s\n", gnutls_strerror(ret_value));
    }
...
bool ocsp_revocation_result(gnutls_ocsp_resp_t ocsp_response)
{
    ...
    /* If cert_status is GNUTLS_OCSP_CERT_REVOKED, then this variable hold gnutls_x509_crl_reason_t enum value */
    unsigned int revocation_reason;

    if ((ret_err_val = gnutls_ocsp_resp_get_single(ocsp_response, index, &hash_algorithm, &issuer_name_hash, &issuer_key_hash, &serial_number, &cert_status, NULL, NULL, &revocation_time, &revocation_reason)) != GNUTLS_E_SUCCESS)
    {
...
static bool ocsp_revoc_check_single_certificate(gnutls_x509_crt_t certificate, gnutls_x509_crt_t issuer_certificate) {
    ...
    ocsp_responder_uri = extract_ocsp_responder_uri(certificate);
    if (ocsp_responder_uri == NULL) {
        return true;
    }
    ...
    /* Send the generated OCSP Request with libcurl to the OCSP Responder and retrieve the response */
    ...
    if (!verify_ocsp_response_signature(ocsp_response, certificate, issuer_certificate, nonce_req)) {
        goto cleanup;
    }
...
{
    printf("\n--- Performing Online Certificate Status Protocol (OCSP) verification! ---\n");

    /* Retrieve the whole server certificate chain */
    size_t chain_size;
    gnutls_x509_crt_t *server_chain_crt = retrieve_server_certificate_chain(session, &chain_size);
    ...
    */
    for (int index = 0; index < chain_size - 1; index++) {
        ...
        if (!ocsp_revoc_check_single_certificate(server_chain_crt[index], server_chain_crt[index+1])) {
            goto cleanup;
        }
    }
