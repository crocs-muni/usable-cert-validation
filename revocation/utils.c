 ...
 *  A function whose task is to examine the result, the status (either
 * successful or unsuccessful) of the certificate verification after the TLS
 * handshake.
 * @param session
 */
void check_result_of_cert_validation(gnutls_session_t session) {
  ...
  /* Prepare a buffer for the error message, fill it, and print the message to
   * the standard error output. */
  gnutls_datum_t datum = {0};
  gnutls_certificate_verification_status_print(cert_verify_status, cert_type,
                                               &datum, 0);
  fprintf(stderr, "%s\n", datum.data);
...
gnutls_x509_crt_t *retrieve_server_certificate_chain(gnutls_session_t session,
  ...
  /* Receive the entire chain of certificates stored in an array where each
   * certificate is in DER format and stored in the gnutls_datum_t structure.
   * The server's certificate is at index 0, its issuer's certificate is at
   * index 1, etc
   */
  ...
  /* Convert certificate array in gnutls_datum_t structure to certificate array
   * in gnutls_crt_t structure */
  ...
  for (int i = 0; i < server_chain_size; i++) {
    ...
    if ((ret_val = gnutls_x509_crt_import(
             server_chain_crt[i], &server_chain_der[i], GNUTLS_X509_FMT_DER)) !=
        GNUTLS_E_SUCCESS) {
      fprintf(stderr,
              "Function 'gnutls_x509_crt_import' at index %d has failed: %s\n",
...
bool print_x509_certificate_info(gnutls_x509_crt_t certificate) {
   ...
   * structure and then written to stdout. */
  gnutls_datum_t certificate_datum;
  if ((ret_err_code = gnutls_x509_crt_print(
           certificate, GNUTLS_CRT_PRINT_ONELINE, &certificate_datum)) !=
      GNUTLS_E_SUCCESS) {
  ...
  gnutls_x509_crt_get_serial(certificate, serial_number_bin,
                             &serial_number_size);
...
bool print_certificate_chain_info(gnutls_session_t session) {
  ...
  const gnutls_datum_t *datum_cert_list =
  ...
  for (int index = 0; index < cert_list_size; index++) {
    ...
    if ((gnutls_x509_crt_import(certificate, &datum_cert_list[index],
