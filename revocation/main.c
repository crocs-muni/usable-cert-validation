...
/*
 * GNUTLS_E_* - macros for error codes, returned value of gnutls_* functions (e.g GNUTLS_E_SUCCESS = 0 )
 *
 * gnutls_certificate_type_t -- GNUTLS_CRT_* (e.g GNUTLS_CRT_X509 )
 * gnutls_certificate_verify_flags -- GNUTLS_VERIFY_* (e.g GNUTLS_VERIFY_DISABLE_CA_SIGN ) (4.1.1.8)
 * gnutls_certificate_status_t -- GNUTLS_CERT_* (e.g GNUTLS_CERT_INVALID ) (4.1.1.7)
 *
 * gnutls_x509_crt_fmt_t -- GNUTLS_X509_FMT_DER or GNUTLS_X509_FMT_PEM
 * gnutls_certificate_print_formats_t -- GNUTLS_CRT_PRINT_* (e.g GNUTLS_CRT_PRINT_FULL )
 *
 * gnutls_trust_list_flags_t -- GNUTLS_TR_* (e.g GNUTLS_TL_VERIFY_CRL )
 *
 * gnutls_x509_crl_reason_flags_t -- GNUTLS_CRL_REASON_* (e.g GNUTLS_CRL_REASON_UNSPECIFIED )
 *
 * gnutls_ocsp_verify_reason_t -- GNUTLS_OCSP_VERIFY_* - verifying the ocsp response of signer
*/
...
void unsecure_print_connection_info(struct addrinfo *act_node)
{
    ...
    if ((ret_status = getnameinfo(act_node->ai_addr, act_node->ai_addrlen, hostname_domain, NI_MAXHOST, service_text, NI_MAXSERV, 0)) != 0)
    {
        fprintf(stderr, "Function 'getnameinfo' has failed: %s\n", gai_strerror(ret_status));
        exit(EXIT_FAILURE);
    }

    if ((ret_status = getnameinfo(act_node->ai_addr, act_node->ai_addrlen, hostname_adress, NI_MAXHOST, service_numeric, NI_MAXSERV, NI_NUMERICHOST | NI_NUMERICSERV)) != 0)
    {
        fprintf(stderr, "Function 'getnameinfo' has failed: %s\n", gai_strerror(ret_status));
        exit(EXIT_FAILURE);
    }
...
int unsecure_connect_to_server(char *hostname)
{
    ...
    for (act_node = adresses; act_node != NULL; act_node = act_node->ai_next)
    {
        ...
        client_fd = socket(act_node->ai_family, act_node->ai_socktype, act_node->ai_protocol);
        if (client_fd == -1)
        {
            printf(" [NOK] \n");
    ...
    {
        printf(" [NOK] \n");
        fprintf(stderr, "A transport layer connection could not be established for the specified hostname on htpps!");
        exit(EXIT_FAILURE);
    }
 ...
 * A callback function that checks the revocation status for all certificates in the certificate chain.
 * This function is configured before the TLS handshake, so that it takes place during the TLS handshake,
 * immediately after the certification chain is verified.
 * The GnuTLS library does not perform revocation checks by default
 * (excluding OCSP-Stapling) (only with leaf server's certificate).
 * @param session
 * @return 0 if handshake should continue, non-zero if handshake should be terminated
 */
int revocation_check_callback(gnutls_session_t session) {
    ...
    /* ORed flags of enum gnutls_certificate_status_t (0 if trusted, non-zero if problem) */
    ...
    /* Retrieve the hostname from the user pointer from the session, set previously */
    hostname = gnutls_session_get_ptr(session);

    /* Verify the peer's certificate and the hostname */
    if (gnutls_certificate_verify_peers3(session, hostname, &certificate_verification_status) != GNUTLS_E_SUCCESS) {
        errx(EXIT_FAILURE, "Function 'gnutls_certificate_verify_peers3' has failed during callback\n");
    }
    ...
    if ((gnutls_certificate_verification_status_print(certificate_verification_status, cert_type, &certificate_verification_status_pretty, 0)) != GNUTLS_E_SUCCESS) {
        errx(EXIT_FAILURE, "Function 'gnutls_certificate_verification_status_print has failed\n'");
    }
    ...
    if (!ct_check_result || !crl_check_result || !ocsp_check_result || !ocsp_stapling_check_result) {
        return GNUTLS_E_CERTIFICATE_ERROR;
    }
...
gnutls_session_t make_secure_connection(int client_fd, int *handshake_result, gnutls_certificate_credentials_t *credentials, char *hostname)
{
    int ret_val;
    ...
    {
        fprintf(stderr, "Function 'gnutls_certificate_allocate_credentials' has failed: %s\n", gnutls_strerror(ret_val));
        goto cleanup;
    }

    /* Add the system's default trusted CAs certificates (as trust anchors) in order to verify the server's certificate. */
    /* Other options are: 'gnutls_certificate_set_x509_trust_dir', 'gnutls_certificate_set_x509_trust_file'. */
    if ((ret_val = gnutls_certificate_set_x509_system_trust(creds)) < 0)
    {
        fprintf(stderr, "Function 'gnutls_certificate_set_x509_system_trust' has failed: %s\n", gnutls_strerror(ret_val));
        goto cleanup;
    }

    /* Uncomment in order to perform offline CRL check of server/client certificates during the TLS handshake. */
    /* Requires the CRL list file to be stored locally. */
//    if ((ret_val = gnutls_certificate_set_x509_crl_file(creds, "test_data/badssl_revoked_CRL.crl", GNUTLS_X509_FMT_DER)) <= 0)
//    {
//        fprintf(stderr, "Function 'gnutls_certificate_set_x509_crl_file' has failed: %s\n", gnutls_strerror(ret_val));
//        goto cleanup;
//    }
    ...
    /* Set the hostname to the session structure, so it can be available during verification callback during TLS handshake */
    ...
    /* Verify the server's certificate with the provided hostname during the TLS handshake. */
    /* Not setting the verification of hostname would mean that we would accept a certificate of any trusted server. */
    ...
    if ((ret_val = gnutls_server_name_set(session, GNUTLS_NAME_DNS, hostname, strlen(hostname))) < 0)
    {
    ...
    /* If no OCSP staple is found, certificate verification during handshake will fail */
    if ((ret_val = gnutls_ocsp_status_request_enable_client(session, NULL, 0, NULL)) != 0)
    {
        fprintf(stderr, "Failed to enable OCSP-Stapling through TLS extension!");
        fprintf(stderr, "Function 'gnutls_ocsp_status_request_enable_client' has failed: %s\n", gnutls_strerror(ret_val));
        goto cleanup;
    }

    /* Associate the credentials structure with the session structure. */
    if ((ret_val = gnutls_credentials_set(session, GNUTLS_CRD_CERTIFICATE, creds)) < 0)
    {
...
//    /* Set custom verification function, which will run immidiately after certificate chain has been received */
     ...
     * returned value <0 - error occured, which could be classified as fatal or non-fatal
    ...
    */
    ...
    if (ret_handshake < 0)
    {
        ...
        /* Examine the gnutls_certificate_status_t enum, set up by verification function (during handshake) */
        //check_result_of_cert_validation(session);
        printf("\n");
    }
...
void close_connection(int client_fd, gnutls_session_t session, gnutls_certificate_credentials_t creds)
{
...
int main(int argc, char **argv)
{
    ...
    gnutls_session_t session = make_secure_connection(client_fd, &handshake_result, &creds, hostname);
