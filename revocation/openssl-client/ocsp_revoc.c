...
char *extract_ocsp_responder_uri(X509 *certificate, int *number_of_responders) {
  ...
  *number_of_responders = ocsp_uris_stack_size;
  ...
  /* Retrieve the first (at index 0) OCSP Responder's uri entry from the
   * STACK_OF(TYPE) array. */
...
int generate_ocsp_request(X509 *certificate, X509 *issuer_certificate,
                          OCSP_CERTID **certid,
                          OCSP_REQUEST **ocsp_request_in) {
  ...
  //        errx(EXIT_FAILURE, "Function 'OCSP_request_add1_cert' has
  //        failed!\n");
  //    }

  /* Create the new OCSP_CERTID structure with default SHA1 message digest
   * (first argument) for given certificate and its issuer. */
  *certid = OCSP_cert_to_id(NULL, certificate, issuer_certificate);
  ...
  /* Structure OCSP_ONEREQ is returned so an application can add additional
  ...
  /* We have ocsp request filled with the server's, issuer's certificate and
   * random nonce */
  *ocsp_request_in = ocsp_request;
  return REVOC_CHECK_SUCCESS;
}

int send_ocsp_request(OCSP_REQUEST *ocsp_request, char *ocsp_uri,
                      OCSP_RESPONSE **ocsp_response_in) {
  CURL *handle = NULL;
  struct curl_slist *headers = NULL;

  /* Prepare the custom (datum_t) structure where the OCSP Response will be
   * placed. */
  struct datum_t ocsp_response_DER = {0};

  /* Convert the previously generated OCSP Request from native OpenSSL structure
   * to the plain DER format. */
  ...
  /* Convert the retrieved OCSP Response from DER format to the native OpenSSL
   * structure. */
  ...
  OCSP_RESPONSE *ocsp_response =
      d2i_OCSP_RESPONSE(NULL, (const unsigned char **)&ocsp_response_der_data,
                        ocsp_response_der_size);
...
int verify_ocsp_response_signature(OCSP_RESPONSE *ocsp_response,
  ...
  /* Check the status of retrieved OCSP response, if it is not malformed or
  ...
  /* This will be required later when verifying signature of OCSP (basic)
   * response and verifying the issuer's certificate as well. */
  ...
  /* Verify the signature of basic OCSP Response with validation of issuer's
   * certificate. */
  /* If we want to just verify the signature of OCSP response and we dont want
   * to validate the server's certificate, use flag OCSP_TRUSTOTHER and the
   * X509_STORE wont be needed. */
...
int parse_revocation_check_from_basic_resp_through_single_resp(
  ...
  /* Retrieve the revocation status of the certificates included in the OCSP
   * Basic response. */
  /* Revocation reason and time will be filled only if revocation_status ==
  ...
  for (int index = 0; index < number_of_single_responses; index++) {
    ...
    /* Retrieve the revocation status, revocation_reason, revocation time and
     * other info */
    /* Similar to functionOCSP_resp_find(), but this one operates on
     * OCSP_SINGLERESP structure. */
    int rev_status = OCSP_single_get0_status(one_response, &revocation_reason,
                                             &rev_time, &thisupd, &nextupd);
...
int ocsp_response_verify_and_check(OCSP_RESPONSE *ocsp_response,
                                   OCSP_CERTID *certid,
  ...
  OCSP_BASICRESP *ocsp_response_basic;
  status = verify_ocsp_response_signature(ocsp_response, cert_chain_stack,
                                          &ocsp_response_basic);
  ...
  if (nonce_check_result == 1) {
    printf(
        "- OK: nonce is present in both the request and the response, and they "
        "are equal!\n");
  } else if (nonce_check_result == 0) {
    fprintf(stderr,
            "- WARNING: nonce is present in both the request and the response, "
            "but they are not equal!\n");
  } else if (nonce_check_result == -1) {
    fprintf(stderr,
            "- WARNING: nonce is present only in the ocsp request, not in the "
            "ocsp response!\n");
  } else if (nonce_check_result == 2) {
    fprintf(
        stderr,
        "- WARNING: nonce is missing in both, the request and the response\n");
  } else if (nonce_check_result == 3) {
    fprintf(stderr,
            "- WARNING: nonce is present only in the ocsp response, not in the "
            "ocsp request!\n");
  }

  /* Get the revocation status of the provided certificate, represented with
   * OCSP_CERTID (with issuer's certificate). */
  /* Revocation reason and time will be filled only if revocation_status ==
   * V_OCSP_CERTSTATUS_REVOKED. */
  int revocation_status;
  int revocation_reason;
  ...
  ASN1_GENERALIZEDTIME *thisupd;
  ASN1_GENERALIZEDTIME *nextupd;
  if (OCSP_resp_find_status(ocsp_response_basic, certid, &revocation_status,
                            &revocation_reason, &rev_time, &thisupd,
                            &nextupd) != 1) {
    fprintf(stderr,
            "- id of the provided certificate was not found in basic OCSP "
            "Response!\n");
    status = REVOC_CHECK_FAILURE;
    goto cleanup;
  }
  ...
  /* Check the validity of this update and next update fields retrieved from
   * basic OCSP Response. */
...
int ocsp_check_single_certificate(X509 *certificate, X509 *issuer_certificate,
                                  STACK_OF(X509) * cert_chain_stack) {
  int ret;
  ...
  if ((ret = generate_ocsp_request(certificate, issuer_certificate, &certid,
                                   &ocsp_request)) != REVOC_CHECK_SUCCESS) {
    return ret;
  }

  /* Save the generated OCSP Request to the file. */
  /* The file can be inspected with command 'openssl ocsp -reqin ocsp_req.der
   * -text' */
  save_OCSP_request_to_file(ocsp_request, "ocsp_req.der");

  /* 3.) Send the generated OCSP Request to the obtained OCSP Responder URL and
   * immediately retrieve the OCSP Response. */
  OCSP_RESPONSE *ocsp_response;
  if ((ret = send_ocsp_request(ocsp_request, ocsp_uri, &ocsp_response)) !=
      REVOC_CHECK_SUCCESS) {
    return ret;
  }

  /* Save the retrieved OCSP Response to the file. */
  /* The file can be inspected with command 'openssl ocsp -respin ocsp_resp.der
   * -text -noverify' */
  save_OCSP_response_to_file(ocsp_response, "ocsp_resp.der");

  /* 4.) Verify the signature of the OCSP Response and check the revocation
   * status of provided certificate. */
  if ((ret = ocsp_response_verify_and_check(ocsp_response, certid, ocsp_request,
                                            cert_chain_stack)) !=
      REVOC_CHECK_SUCCESS) {
    return ret;
  }
...
int ocsp_check(SSL *s_connection) {
  ...
  for (int index = 0; index < cert_chain_stack_size - 1; index++) {
    ...
    ocsp_check_single_certificate(certificate, issuer_certificate,
                                  cert_chain_stack);
  }
