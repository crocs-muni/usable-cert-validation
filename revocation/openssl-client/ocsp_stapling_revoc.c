...
int ocsp_stapling_check(SSL *s_connection) {
  ...
  long ocsp_response_stapled_size =
      SSL_get_tlsext_status_ocsp_resp(s_connection, &ocsp_response_stapled_DER);
  if (ocsp_response_stapled_size == -1) {
    fprintf(stderr, "- server did not send the stapled OCSP Response!\n");
    return REVOC_CHECK_SUCCESS;
  }

  /* Convert the retrieved stapled OCSP Response to the OpenSSL native
   * structure. */
  OCSP_RESPONSE *stapled_ocsp_response = d2i_OCSP_RESPONSE(
      NULL, (const unsigned char **)&ocsp_response_stapled_DER,
  ...
  status = verify_ocsp_response_signature(
      stapled_ocsp_response, cert_chain_stack, &stapled_ocsp_response_basic);
  if (status != REVOC_CHECK_SUCCESS) {
    goto cleanup;
  }

  /* Find out the revocation status for every certificate included in the
   * stapled OCSP Response. */
  status = parse_revocation_check_from_basic_resp_through_single_resp(
      stapled_ocsp_response_basic);
  if (status != REVOC_CHECK_SUCCESS) {
    goto cleanup;
  }
