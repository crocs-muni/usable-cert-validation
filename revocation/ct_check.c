...
static bool ct_check_single_timestamp(gnutls_datum_t logid, gnutls_sign_algorithm_t sigalg, gnutls_datum_t signature, time_t timestamp) {
...
static bool ct_check_single_certificate(gnutls_x509_crt_t certificate) {
    ...
    ret_err_val = gnutls_x509_crt_get_extension_by_oid2(certificate, CT_SCT_OID, index, &sct_list_DER, &critical);
    if (ret_err_val != GNUTLS_E_SUCCESS)
    {
        ...
        fprintf(stderr, "Function 'gnutls_x509_crt_get_extension_by_oid2' has failed: %s\n", gnutls_strerror(ret_err_val));
        exit(EXIT_FAILURE);
    }

    /* Convert the DER encoded CT SCT list from gnutls_datum_t structure to native gnutls_x509_ct_scts_t structure. */
    if ((ret_err_val = gnutls_x509_ext_ct_scts_init(&sct_list)) != GNUTLS_E_SUCCESS)
    {
        fprintf(stderr, "Function 'gnutls_x509_ext_ct_scts_init' has failed: %s\n", gnutls_strerror(ret_err_val));
        goto cleanup;
    }
    if ((ret_err_val = gnutls_x509_ext_ct_import_scts(&sct_list_DER, sct_list, 0)) != GNUTLS_E_SUCCESS)
    {
        fprintf(stderr, "Function 'gnutls_x509_ext_ct_import_scts' has failed: %s\n", gnutls_strerror(ret_err_val));
        goto cleanup;
    }
    ...
    for (int index = 0; ; index++) {
        ...
        ret_err_val = gnutls_x509_ct_sct_get(sct_list, index, &timestamp, &logid, &sigalg, &signature);
